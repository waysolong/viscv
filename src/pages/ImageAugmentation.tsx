...
import {
  FolderOpenOutlined, UndoOutlined, RedoOutlined, ExportOutlined, ClearOutlined, ExperimentOutlined,
} from "@ant-design/icons";
import { useAugment } from "../lib/stores";
import { AUGMENT_TYPES } from "../lib/enhancements";
...
import type { EnhancementType, ImageInfo } from "../types";

function emptyHistograms() {
  return { gray: [], red: [], green: [], blue: [] };
}

/** 独立于「图像工作台」并列的图像增强模块：Ultralytics 风格增强算子。 */
export default function ImageAugmentation() {
  const editor = useAugment();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (useAugment.getState().originalPath) return;
        const path = await backend.defaultImagePath();
        if (cancelled || useAugment.getState().originalPath) return;
        const info = await backend.loadImage(path);
        if (!cancelled) useAugment.getState().setOriginal(path, info);
      } catch (e) {
        console.error("[viscv] 默认示例图加载失败", e);
      }
    })();
  ...
  const onLoad = async (path: string) => {
    try {
      editor.setOriginal(path, await backend.loadImage(path));
    } catch (e) { msg.error("加载失败：" + String(e)); }
  };
  ...
  const onPick = async () => {
    ...
    } catch (e) { msg.error("打开图像失败：" + String(e)); }
  };
  ...
  const onExport = async () => {
    ...
    const out = await backend.pickSavePath("viscv-augment.png");
    if (!out) return;
    editor.setBusy(true);
    try {
      await backend.exportImage(editor.originalPath, editor.steps, out);
      msg.success("导出成功：" + out);
    } catch (err) { msg.error("导出失败：" + String(err)); } finally { editor.setBusy(false); }
  };
    ...
    <div className="flex h-full flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <Space>
          ...
          <span className="text-gray-500">深度学习图像增强</span>
        </Space>
      </div>
      ...
      {!hasImage ? (
        <PageCard className="flex-1">
          <EmptyState
            description="加载一张图像，体验 Ultralytics 风格的图像增强算子"
        ...
        <div className="grid flex-1 gap-3" style={{ gridTemplateColumns: "360px 1fr" }}>
          <PageCard title="增强管线" className="overflow-hidden" extra={editor.steps.length ? `共 ${editor.steps.length} 步` : undefined}>
            <StepList
              steps={editor.steps}
